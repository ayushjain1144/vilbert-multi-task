...
class FeatureExtractor:
    ...
    def __init__(self):
        ...
        self.pix_T_cam = torch.Tensor(INTRINSICS).unsqueeze(0)  
    ...
    def get_pointnet_features(self, scan_paths):
        ...
        for scan_path in scan_paths:
            ...
            for obj_id in range(len(scan.three_d_objects)):
                ...
                st()
                obj_img = utils_geom.apply_pix_T_cam(
                                    self.pix_T_cam,
                                    torch.from_numpy(point_cloud).unsqueeze(0).float()
                                    )
