...
import utils.utils_samp as utils_samp
...
class Vox_util(object):
    def __init__(self, Z, Y, X, set_name, scene_centroid, bounds=None, assert_cube=True):
        ...
        if bounds is None:
            self.XMAX = 16.0 # right
            self.YMIN = -16.0 # down (neg is up)
            self.YMAX = 16.0 # down
            self.ZMIN = -16.0 # forward
            self.XMIN = -16.0 # right (neg is left)
            self.ZMAX = 16.0 # forward
    ...
    def apply_ref_T_mem_to_lrtlist(self, lrtlist_mem, Z, Y, X, assert_cube=True):
        ...
        # apply_4x4 will work for the t part
        lenlist_mem, rtlist_mem = utils_geom.split_lrtlist(lrtlist_mem)
        __p = lambda x: utils_basic.pack_seqdim(x, B)
        __u = lambda x: utils_basic.unpack_seqdim(x, B)
        rlist_mem_, tlist_mem_ = utils_geom.split_rt(__p(rtlist_mem))
        # rlist_cam_ is B*N x 3 x 3
        # tlist_cam_ is B*N x 3
        tlist_cam_ = __p(utils_geom.apply_4x4(cam_T_mem, __u(tlist_mem_)))
        # rlist does not need to change, since cam is aligned with mem
        rlist_cam_ = rlist_mem_.clone()
        rtlist_cam = __u(utils_geom.merge_rt(rlist_cam_, tlist_cam_))
        # this is B x N x 4 x 4

        # next we need to scale the lengths
        lenlist_mem, _ = utils_geom.split_lrtlist(lrtlist_mem)
        # this is B x N x 3
        vox_size_X = (self.XMAX-self.XMIN)/float(X)
        vox_size_Y = (self.YMAX-self.YMIN)/float(Y)
        vox_size_Z = (self.ZMAX-self.ZMIN)/float(Z)
        ...
        lenlist_cam = torch.cat([xlist * vox_size_X,
                                 ...
                                 zlist * vox_size_Z], dim=2)
        # merge up
        lrtlist_cam = utils_geom.merge_lrtlist(lenlist_cam, rtlist_cam)
        return lrtlist_cam
    ...
    def crop_zoom_from_mem(self, mem, lrt, Z2, Y2, X2, additive_pad=0.0, mode='bilinear'):
        ...
        zoom = utils_samp.sample3d(mem, xyz_mem, Z2, Y2, X2, mode=mode)
